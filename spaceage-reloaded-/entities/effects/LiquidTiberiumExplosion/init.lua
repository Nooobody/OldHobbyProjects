...
matRefraction:SetInt("$spriterendermode",9)
local tMats = {}
...
function EFFECT:Init( data )
	...
	self:EmitSound( "ambient/explosions/explode_9.wav", 130,130 )
	...
	for i=1, math.ceil(26) do
		...
		local spawnpos = Pos + 64*vecang
			...
			particle:SetColor(20, math.random(100,255),20)
	...
	for i=1, math.ceil(26) do
		...
		local spawnpos = Pos + 256*vecang
			...
			for k=5,26 do
			local particle = self.Emitter:Add( "particles/flamelet"..math.random(1,5), spawnpos - vecang*9*k)
			particle:SetVelocity(vecang*math.Rand(2,30))
			...
			particle:SetRoll( math.Rand( 20, 80 ) )
			particle:SetRollDelta( math.random( -1, 1 ) )
			particle:SetColor(100, math.random(230,255),100)
			particle:VelocityDecay( true )
			end
	...
	if timeleft > 0 then 
		...
		if (self.Size < 8000) then
			...
			for i=1, NumPuffs do
					...
					particle:SetVelocity(newang*math.Rand(0,3))
...
local startpos = self.Position
	...
	render.DrawSprite(startpos, 400*self.GSize,90*self.GSize,Color(80, math.random(230,255), 80),self.GAlpha)
	render.DrawSprite(startpos, 70*self.GSize,280*self.GSize,Color(80, math.random(240,255), 80),0.7*self.GAlpha)
	--shockwave
