...
net.Receive("SA_ObjectLooked",function(len,ply)
	...
	if not Ent.SentData[ply:EntIndex()] or Ent.SentData[ply:EntIndex()].NeedsUpdate then
		...
		net.Start("SA_ObjectLookedReceive")
			...
			if Ent:GetClass() == "sa_mining_icelaser" or Ent:GetClass() == "sa_mining_refinery" then
				if Ent.SentData[ply:EntIndex()].Percent ~= Ent.Percent then
					State(S_PERCENT,"Percent")
					net.WriteUInt(Ent.Percent,8)
					Ent.SentData[ply:EntIndex()].Percent = Ent.Percent
				end
			end
