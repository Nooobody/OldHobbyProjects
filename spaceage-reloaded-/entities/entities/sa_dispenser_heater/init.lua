...
function ENT:ThinkStart()
	if not next(self.StoredLinks.Inputs) or not next(self.StoredLinks.Inputs.Energy) or not next(self.StoredLinks.Inputs.Steam) then return end
	...
	while I <= #self.PlysUsing do
		...
		elseif P:GetPos():Distance(self:GetPos()) > RANGE or P:GetPos().z < self:GetPos().z or P.Ice == 0 then
			table.remove(self.PlysUsing,I)
	...
	for I,P in pairs(player.GetAll()) do
		if P:GetPos():Distance(self:GetPos()) < RANGE and P:GetPos().z > self:GetPos().z and P.Ice > 0 then
...
function ENT:LifeSupport()
	if not table.HasValue(self.Got,"Steam") then return end
	for I,P in pairs(self.PlysUsing) do
		if IsValid(P) and P.Ice > 0 then
			P.Ice = P.Ice - 1
			P.Steam = P.Steam + 1
		end
		net.Start("PlayerSurv")
