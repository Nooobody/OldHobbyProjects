TOOL.Mode 		= "sa_ls_exhaler"
TOOL.Tab 		= "SA"
TOOL.Category 	= "LS"
TOOL.Name		= "Gas Ventilator"
TOOL.Command	= nil
TOOL.ConfigName	= ""

TOOL.ClientConVar["Class"] = "sa_exhaler_oxygen"
TOOL.ClientConVar["Size"] = 1
TOOL.ClientConVar["Frozen"] = 0
TOOL.ClientConVar["Multiplier"] = 2

if CLIENT then
	language.Add( "tool.sa_ls_exhaler.name", "Ventilator Tool" )
	language.Add( "tool.sa_ls_exhaler.desc", "Left-click to spawn Ventilators." )
	language.Add( "tool.sa_ls_exhaler.0", "Left Click: Spawn a Gas Ventilator    Right Click: Spawn without Weld" )
	language.Add( "Undone_Atmosphere Exhaler", "Undone Gas Ventilator")
end

cleanup.Register("sa_exhaler_oxygen")
cleanup.Register("sa_exhaler_methane")
cleanup.Register("sa_exhaler_hydrogen")
cleanup.Register("sa_exhaler_nitrogen")
cleanup.Register("sa_exhaler_sulfur")
cleanup.Register("sa_exhaler_carbon_dioxide")
...
function TOOL:RightClick(trace)
	...
	undo.Create("Atmosphere Exhaler")
		undo.AddEntity(Ent)
...
function TOOL.BuildCPanel(Panel)
	...
	Ch:SetConVar("sa_ls_exhaler_Frozen")
	Ch:SetText("Frozen?")
	Ch:SizeToContents()
	Ch:Dock(TOP)
	local Slid = vgui.Create("DNumSlider")
	Slid:SetConVar("sa_ls_exhaler_Multiplier")
	Slid:SetValue(20)
	Slid:SetMin(1)
	Slid:SetMax(100)
	Slid:SetDecimals(0)
	...
	for I,P in pairs(list.Get("sa_exhaler")) do
		local No = VG:AddNode(P)
		for i,p in pairs(list.Get("sa_exhaler_"..string.lower(P))) do
			local Btn = No:AddNode(A[i])
			Btn.Icon:SetImage("icon16/page.png")
			Btn.DoClick = function(btn)
				RunConsoleCommand("sa_ls_exhaler_Class","sa_exhaler_"..string.lower(P))
				RunConsoleCommand("sa_ls_exhaler_Size",i)
			end
		end
	end
	Panel:AddItem(VG)
